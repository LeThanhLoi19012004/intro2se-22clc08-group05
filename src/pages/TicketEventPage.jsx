...
import { renderEvent, renderEventPost, followEvent, OrderTicket, CancelTicket, StatusOrder, JoinEvent} from "../API";
    ...
    const fetchEvents = async () => {
      try {
        ...
        if (response.success) {
          ...
          setNumParticipant(response.data.participants - response.data.participants_id.length)
          if (response.data.participants_id.includes(profile) ){
            setJoinedText("Cancel Participation");
            setJoinBackGround("linear-gradient(150deg, #2d158f, #ff7383)");
          }
          // Thực hiện các logic liên quan đến infoEvent
          if (response.data.eventtype === "Music" || response.data.eventtype === "Festival") {
            setTicket(true);
  ...
  useEffect(() => {
    if (!loading) {
      const interval = setInterval(() => {
        const now = new Date();
        const target = new Date(infoEvent.eventdate);
        ...
        if (difference <= 0) {
          ...
          setTimeRemaining(
            `${days} days ${hours}h ${minutes}m ${seconds}s left!`
          );
        }
      }, 1000);
        ...
        <div className="ticket-event-page__cols__container">
            ...
            <div className="ticket-event-page__layer">
              <div className="ticket-event-page__img__container">
                <div className="ticket-event-page__round">
                  <img src={infoEvent.logoevent ? `data:${infoEvent.logoevent.contentType};base64,${infoEvent.logoevent.imageBase64}` : "#"} alt="User img" />
                </div>
                <span></span>
              </div>
              ...
              <div className="organizer-ticket-event-page__upper-part">
                ...
                <ul className="ticket-event-page__remain-info">
                  <li>
                    <span>Date:</span>{timeRemaining}
                  </li>
                  <li>
                    <span>Time:</span>This is data
                  </li>
                  <li>
                    <span>Category:</span>{infoEvent.eventtype}
                  </li>
                  <li>
                    <span>Location:</span>{infoEvent.location}
                  </li>
                </ul>
              </div>
          ...
          <div className="ticket-event-page__right__col">
                ...
                >
                  ...
                  >
                    ...
                    <div className="ticket-event-page__dropdown-agreement">
                      ...
                      <p>
                        I agree with the Terms and Conditions of the Suki Event
                        Management Platform and am responsible for the
            ...
            <div className="ticket-event-page__container-post">
              {isOrganizer ? <MyPostWidget /> : <div></div>}
