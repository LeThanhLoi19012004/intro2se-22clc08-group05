...
const PostCarousel = ({ images, setLiked,liked }) => {
  ...
  const handleDoubleClick = () => {
    setShowHeart(true);
    if (!liked) { // Only trigger like if not already liked
      setLiked(true);
    }
    setTimeout(() => setShowHeart(false), 500); // Heart icon will disappear after 0.5s
  };
      ...
      {images.length > 0 && (
        <>
          ...
          {images.length > 1 && (
           ...
           {showHeart && (
            <AiFillHeart size = {100} className={`heart-icon ${showHeart ? 'show' : ''}`} />
          )}
        </>
      )}
    </div>
    ...
    <div className="post__post">
      ...
      <div className="post__post-activity">
        ...
        <div className="post__post-activity-link">
          <FaRegCommentDots size={27}  onClick={() => setShowOverlay(!showOverlay)}/>
