...
const DeleteAccount = async (req, res) =>
  {
    try {
      ...
      if (!profileID || !password) {
        return res.status(400).json({success: false, message: 'ProfileID and password are required'});
      }
      ...
      if (checkOrganizer) {
        return res.status(400).json({success: false, message: 'Cannot delete profile because it is associated with an event'});
      }
