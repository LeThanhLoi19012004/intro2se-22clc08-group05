  ...
  try {
    ...
    if (posts.length === 0) {
      ...
      const postsData = posts.map(post => {
        ...
        postObj.postcreationdateFormatted = post.postcreationdate ? new Date(post.postcreationdate).toDateString() : 'No date available';
...
const RenderMPPost = async (req, res) => {
  try {
    const posts = await PostModel.find()
      .populate('eventID', 'eventname logoevent')
      .exec();
    
    if (posts.length === 0) {
      return res.status(400).json({ success: false, message: 'No posts available' });
    } else {
