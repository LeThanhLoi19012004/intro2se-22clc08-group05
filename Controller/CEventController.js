...
const RenderEvent = async (req, res) => {
  ...
  try {
    ...
    if (!findEvent) {
      ...
      res.json({success: true, data: findEvent, isOrga: isOrga, follow: follow})
    }
  } catch (error) {
    console.error(error);
...
const SearchEvent = async (req, res ) => {
  try {
    const events = await CEventModel.find({status: "Approved"}, {
      _id: 1,
      logoevent: 1,
      eventname: 1,
      eventtype: 1,
  ...
  try {
    ...
    if (!findEvent) {
      ...
      if (findEvent.follows.includes(profile)){
        findEvent.follows = findEvent.follows.filter(item => !item.equals(profile));
      }
      else {
  ...
  try {
    ...
    if (!findEvent) {
      ...
      if (findEvent.participants_id.includes(profile)){
        ...
        findEvent.participants_id = findEvent.participants_id.filter(item => !item.equals(profile));
      }
      else {
...
const RenderEventMP = async (req, res) => {
  ...
  try {
    ...
    const event_follow = events.filter(event => event.follows.includes(profile));
    const event_orga = events.filter(event => event.profile == profile);
    res.json({success: true, event_follow: event_follow, event_orga: event_orga});
  } catch (error) {
    console.error(error);
    res.status(500).json({message: 'Server error'});
  }
...
const AdminRenderEvent = async (req, res) => {
  try {
    const events = await CEventModel.find({}, {
      ...
      status: 1,
      eventcreationdate: 1,    
    }).populate('profile', 'fullname avatar').exec();
    res.json({success: true, data: events});
  } catch (error) {
    console.error(error);
    res.status(500).json({message: 'Server error'});
